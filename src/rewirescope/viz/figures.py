...
def phenology_ribbons(
    ...
    fig.update_layout(
        ...
        xaxis_title="Sequence (year-week)",
        yaxis_title="Taxon",
    )
    return fig
...
def silent_edge_heatmap(silent: pd.DataFrame, site_id: str, year: int, max_plants: int = 40) -> go.Figure:
    ...
    fig = go.Figure(
        data=go.Heatmap(
            ...
            x=list(matrix.columns),
            y=list(matrix.index),
            colorscale=[[0, "#F4F2F8"], [1, COLORS["silent-edge-failure"]]],
    ...
    fig.update_layout(
        ...
        margin=dict(l=10, r=10, t=35, b=10),
        template=TEMPLATE,
        title="Candidate Silent-Edge Flags by Flowering Plant",
        xaxis_title="Week",
        yaxis_title="Plant",
    )
    return fig
...
def concentration_small_multiples(snapshots: pd.DataFrame, site_id: str, year: int) -> go.Figure:
    ...
    if df.empty:
        return empty_figure()
    metrics = [
        ("dependency_gini", "Dependency Gini"),
        ("top10_interaction_share", "Top-10 Share"),
        ("effective_taxa_load", "Effective Taxa Load"),
        ("connectance", "Connectance"),
        ("nestedness_proxy", "Nestedness Proxy"),
        ("specialization_proxy", "Specialization Proxy"),
    ]
    fig = go.Figure()
    for col, label in metrics:
        ...
        fig.add_trace(go.Scatter(x=df["time_bin"], y=scaled, mode="lines+markers", name=label))
    fig.update_layout(
        height=460,
        margin=dict(l=62, r=22, t=58, b=108),
        template=TEMPLATE,
        title="Structural Signals (scaled within selected year)",
        paper_bgcolor=PAPER_BG,
        plot_bgcolor=PLOT_BG,
        font=dict(color=FONT_COLOR, size=12),
        yaxis_title="Scaled value",
        xaxis_title="Week",
        legend=dict(orientation="h", yanchor="top", y=-0.27, xanchor="left", x=0, font=dict(size=11)),
    )
    fig.update_xaxes(tickangle=35, gridcolor=GRID_COLOR, zerolinecolor=GRID_COLOR)
...
def concentration_radar(snapshots: pd.DataFrame, site_id: str, time_bin: str) -> go.Figure:
    ...
    fig = go.Figure(
        data=go.Scatterpolar(
            ...
            fill="toself",
            line=dict(color=COLORS["silent-edge-failure"]),
            name=time_bin,
        )
    )
    fig.update_layout(
...
def burkle_contrast_network(contrast: pd.DataFrame, statuses: list[str] | None = None, max_edges: int = 180) -> go.Figure:
    ...
    if df.empty:
        return empty_figure()
    status_order = {"lost": 0, "persisted": 1, "novel_contemporary": 2}
    df["status_order"] = df["edge_status"].map(status_order).fillna(99)
    df = df.sort_values(["status_order", "source_taxon_id", "target_taxon_id"]).head(max_edges)
    plants = sorted(df["source_taxon_id"].unique())
    bees = sorted(df["target_taxon_id"].unique())
    pos = {}
    for idx, node in enumerate(plants):
        pos[node] = (0, 1 - idx / max(1, len(plants) - 1))
    for idx, node in enumerate(bees):
        pos[node] = (1, 1 - idx / max(1, len(bees) - 1))
    fig = go.Figure()
    color_map = {"lost": COLORS["death"], "persisted": COLORS["green"], "novel_contemporary": COLORS["yellow"]}
    for status in ["lost", "persisted", "novel_contemporary"]:
        subset = df[df["edge_status"] == status]
        for row in subset.itertuples(index=False):
            x0, y0 = pos[row.source_taxon_id]
            x1, y1 = pos[row.target_taxon_id]
            fig.add_trace(
                go.Scatter(
                    ...
                    y=[y0, y1],
                    mode="lines",
                    line=dict(width=1.4, color=color_map[status]),
                    opacity=0.65,
                    name=status,
                    legendgroup=status,
                    showlegend=not any(trace.name == status for trace in fig.data),
                    hovertemplate=f"{row.source_taxon_id} -> {row.target_taxon_id}<br>{status}<extra></extra>",
                )
            )
    node_x, node_y, node_color, node_text = [], [], [], []
    for node in plants:
    ...
    fig.update_layout(
        ...
        margin=dict(l=10, r=10, t=35, b=10),
        template=TEMPLATE,
        title="Burkle Historical Contrast Network (partial contrast)",
        xaxis=dict(showticklabels=False, showgrid=False, zeroline=False, range=[-0.08, 1.08]),
        yaxis=dict(showticklabels=False, showgrid=False, zeroline=False),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
    )
